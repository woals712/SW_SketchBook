  ...
  <div class="study">
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-8">
          <!-- 블로그 내용 -->
          <h2 style="color:#34495e;padding-bottom:30px;">카테고리 검색 결과 : '{{ $route.params.category }}'</h2>
          ...
          <ul class="pagination justify-content-center mb-4">
            <li class="page-item">
              ...
              <button class="page-link" v-if="pageData.page > 1"><router-link :to="{ name: 'categoryArticle', params: { tag: $route.params.category }, query: { page : beforepage }}">&larr; 이전</router-link></button>
            </li>
            ...
            <li class="page-item">
              ...
              <button class="page-link" v-if="pageData.page < pageData.endPage"><router-link :to="{ name: 'categoryArticle', params: { tag: $route.params.category }, query: { page : nextpage }}">다음 &rarr;</router-link></button>
            </li>
          </ul>
        </div>
            ...
            <div class="card-body">
              <div v-for="category in myCategory" :key="category.id">
                <div class="btn-group btn-block">
                  <button v-if="category.name === $route.params.category" type="button" class="btn btn-dark d-flex justify-content-between" aria-haspopup="true" aria-expanded="false">
                    <router-link :to="{ name: 'categoryArticle', params: { category: category.name }}" class="text-white">{{ category.name }}({{ category.count }})</router-link>
                    <button v-if="tempname" class="plusB btn btn-danger btn-sm" @click="deletearticle(category.name, category.id)">삭제</button>
                  </button>
                  <button v-else type="button" class="btn btn-outline-secondary d-flex justify-content-between" aria-haspopup="true" aria-expanded="false">
...
export default {
          ...
          axios.delete(`/study/${id}`)
            .then(res => {
              ...
              if (this.$route.query.page * 1 > 1) {
                axios.get(`/${this.$store.state.username}/study/category/${this.$route.params.category}?page=${this.$route.query.page}`)
                  .then(res => {
                    this.myStudy = res.data.studyList
                    this.pageData = res.data.pageData
                  })
              } else {
                axios.get(`/${this.$store.state.username}/study/category/${this.$route.params.category}`)
                  .then(res => {
                    this.myStudy = res.data.studyList
                    this.pageData = res.data.pageData
                  })
  ...
  created () {
    if (this.$store.state.wantuser) {
      if (this.$route.query.page * 1 > 1) {
        axios.get(`/${this.$store.state.wantuser}/study/category/${this.$route.params.category}?page=${this.$route.query.page}`)
          .then(res => {
            this.myStudy = res.data.studyList
            this.pageData = res.data.pageData
          })
      } else {
        axios.get(`/${this.$store.state.wantuser}/study/category/${this.$route.params.category}`)
          .then(res => {
            this.myStudy = res.data.studyList
            this.pageData = res.data.pageData
          })
