  ...
  <div class="container">
    <div class="form-v4-content">
      ...
      <div class="form-left">
        ...
        <br>
        <h2>카드 미리보기</h2>
        <img class="card" :src="imageFunction"/>
      </div>
      ...
      <form class="form-detail" @submit.prevent="submitForm">
        ...
        <div class="form-group">
          <div class="form-row form-row-1">
            ...
            <input v-model="projectInfo.title" type="text" class="input-text" id="titleInput" placeholder="프로젝트 제목">
          </div>
          <div class="form-row form-row-1">
...
사이트 : GitHub, YouTube"></textarea>
        ...
        <div class="form-row-last">
          <button type="submit" class="btn btn-primary" @click="CreateProject">생성</button>
        </div>

        <img id="outImage" :src="projectInfo.outerImage" style="display:none;"/>
        <img id="inImage" :src="projectInfo.innerImage" style="display:none;"/>
...
export default {
  ...
  computed: {
    imageFunction: function () {
      for (var t = 0; t < this.projectInfo.imageMerge.length; t++) {
        if (this.projectInfo.outerImage === this.projectInfo.imageMerge[t]) {
          return this.projectInfo.imageMerge[t - 5]
        }
      }
      if (Number(this.projectInfo.outerImage) >= 1 && Number(this.projectInfo.outerImage) <= 5) {
        return this.projectInfo.imageMerge[Number(this.projectInfo.outerImage) - 1]
      } else if (this.projectInfo.outerImage) {
        return this.projectInfo.outerImage
      }
      return this.projectInfo.imageMerge[0]
    }
  },
  methods: {
    CreateProject () {
      if (!this.projectInfo.title) {
        ...
        if (!this.projectInfo.outerImage) {
          ...
          this.outerNum = true
        } else if (Number(this.projectInfo.outerImage) >= 1 && Number(this.projectInfo.outerImage) <= 5) {
          this.projectInfo.outerImage = this.projectInfo.imageMerge[Number(this.projectInfo.outerImage) - 1 + 5]
          this.outerNum = true
        } else {
        ...
        if (!this.projectInfo.innerImage) {
          ...
          this.innerNum = true
        } else if (Number(this.projectInfo.innerImage) >= 1 && Number(this.projectInfo.innerImage) <= 5) {
          this.projectInfo.innerImage = this.projectInfo.imageMerge[Number(this.projectInfo.innerImage) - 1 + 5]
          this.innerNum = true
        } else {
        ...
        if (this.innerNum && this.outerNum) {
          axios.post('/portfolio', this.projectInfo)
            .then(res => {
