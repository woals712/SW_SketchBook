<template>
  <div class="header-wrap">
    <header>
      <h1 class="brand">
        <router-link to="/">
          <img :src="`userdata/logo.png`">
          <span>SwSketchbook</span>
        </router-link>
      </h1>
      <nav class="header-menu">
        <ul>
          <li v-for="(item, index) in navigation" :key="index" :class="{'nav-dropdown-container': item.type === 'dropdown'}">
            <span v-if="item.type === 'dropdown' && !item.path && isLogin && username" v-text="username + '님'"></span>
            <span v-if="item.type === 'dropdown' && !item.path && isLogin && !username" v-text="item.label"></span>
            <span v-if="item.type === 'dropdown' && !item.path && !isLogin" v-text="item.label"></span>
            ...
            <ul v-if="item.type === 'dropdown'" class="nav-dropdown">
              <li v-for="(child, index) in item.children" :key="index">
                <a v-if="child.type === 'outgoing'" :href="child.link" v-text="child.label" target="_self" rel="noopener noreferrer"></a>
                <router-link v-if="child.type !== 'outgoing'" :to="child.path" v-text="child.label"></router-link>
              </li>
            </ul>
            <a v-if="item.type === 'outgoing'" :href="item.link" target="_self" rel="noopener noreferrer" v-text="item.label"></a>
    ...
    <div class="side-menu" :class="{'side-menu-open': menu}">
      <ul>
        <li class="side-label" v-for="(item, index) in navigation" :key="index" :class="{'side-dropdown-container': item.type === 'dropdown'}">
            <span v-if="item.type === 'dropdown' && !item.path && isLogin && username" v-text="username + '님'"></span>
            <span v-if="item.type === 'dropdown' && !item.path && isLogin && !username" v-text="item.label"></span>
            <span v-if="item.type === 'dropdown' && !item.path && !isLogin" v-text="item.label"></span>
          <router-link v-if="item.type === 'dropdown' && item.path" :to="item.path" v-text="item.label"></router-link>
...
export default {
  ...
  computed: {
    ...mapState(['isLogin', 'username', 'tempname']),
    usernamefunction () {
    ...
    navigation () {
      ...
      if (this.isLogin) {
        // const navigation = this.$store.getters.navigationNonuser
        // navigation[4].label = this.username
        // return navigation
        return this.$store.getters.navigationUser
      } else {
        return this.$store.getters.navigationNonuser
      }
    }
  },
  methods: {
