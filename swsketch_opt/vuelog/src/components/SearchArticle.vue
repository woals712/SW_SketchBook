  ...
  <div class="study">
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-8">
          <!-- 블로그 내용 -->
          <h2 style="color:#34495e;padding-bottom:30px;">검색어 결과 : '{{ $route.params.search }}'</h2>
          ...
          <ul class="pagination justify-content-center mb-4">
            <li class="page-item">
              <button class="page-link" v-if="pageData.page > 1"><router-link :to="{ name: 'searchArticle', params: { tag: $route.params.search }, query: { page : beforepage }}">&larr; 이전</router-link></button>
            </li>
            <p class="my-2 mx-2" v-if="pageData.listCnt != 0 && myStudy">{{ pageData.page }}/{{ pageData.endPage }}</p>
            <li class="page-item">
              <button class="page-link" v-if="pageData.page < pageData.endPage"><router-link :to="{ name: 'searchArticle', params: { tag: $route.params.search }, query: { page : nextpage }}">다음 &rarr;</router-link></button>
            </li>
          </ul>
        </div>
...
export default {
          ...
          axios.delete(`/study/${id}`)
            .then(res => {
              ...
              if (this.$route.query.page * 1 > 1) {
                axios.get(`/${this.$store.state.username}/study/search/${this.$route.params.search}?page=${this.$route.query.page}`)
                  .then(res => {
                    this.myStudy = res.data.searchList
                    this.pageData = res.data.pageData
                  })
              } else {
                axios.get(`/${this.$store.state.username}/study/search/${this.$route.params.search}`)
                  .then(res => {
                    this.myStudy = res.data.searchList
                    this.pageData = res.data.pageData
                  })
  ...
  created () {
    if (this.$store.state.wantuser) {
      if (this.$route.query.page * 1 > 1) {
        axios.get(`/${this.$store.state.wantuser}/study/search/${this.$route.params.search}?page=${this.$route.query.page}`)
          .then(res => {
            this.myStudy = res.data.searchList
            this.pageData = res.data.pageData
          })
      } else {
        axios.get(`/${this.$store.state.wantuser}/study/search/${this.$route.params.search}`)
          .then(res => {
            this.myStudy = res.data.searchList
            this.pageData = res.data.pageData
          })
