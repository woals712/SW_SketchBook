      ...
      <form class="form-detail" @submit.prevent="submitForm">
        ...
        <div class="form-group">
          <div class="form-row form-row-2">
            ...
            <input v-model="articleInfo.title" type="text" class="input-text mb-0" id="titleInput" placeholder="게시글 제목">
        ...
        <div class="form-row">
          ...
          <textarea v-model="articleInfo.content" class="mb-0" id="detailArea" rows="10" style="resize: none;margin-bottom: 25px;"
        ...
        <div class="form-group">
          <div class="form-row form-row-2">
            ...
            <select v-model="articleInfo.category" class="form-control mb-0" @change="changeJobTitle($event)" id="categoryInput" style="resize: none;margin-bottom: 25px;">
        ...
        <button @click="UpdateArticle" type="button" class="btn btn-primary">작성하기</button>
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    submitForm () {
      ...
      updateArticle.update(this.form).then(() => {
        this.$swal({
          icon: "success",
          title: "글 수정 완료!",
        })
        ...
        this.errorMessage = 'Failed to update article. ' + error.message
      })
    },
    UpdateArticle () {
      this.$v.$touch()
      ...
      // this.articleInfo.tagData = this.articleInfo.tagNameList
      axios.put(`/study/${this.$route.params.id}`, this.articleInfo)
        .then((res) => {
  ...
  created () {
    ...
    axios.get(`/study/${this.$route.params.id}`)
      .then(res => {
        this.articleInfo = res.data.StudyData
        this.articleInfo.tagData = res.data.StudyData.tagNameList
      })
  }
}
</script>
