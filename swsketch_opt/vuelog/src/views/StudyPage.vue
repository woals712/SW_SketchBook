<template>
  <div class="study">
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-8">
          <h2 style="color:#34495e;padding-bottom:30px;">STUDY 게시글</h2>
          <!-- 블로그 내용 -->
          ...
          <ul class="pagination justify-content-center mb-4">
            <li class="page-item">
              <a class="page-link" v-if="pageData.page > 1" :href="`/study?page=${beforepage}`">&larr; 이전</a>
            </li>
            <p class="my-2 mx-2" v-if="pageData.listCnt != 0">{{ pageData.page }}/{{ pageData.endPage }}</p>
            <li class="page-item">
              <a class="page-link" v-if="pageData.page < pageData.endPage" :href="`/study?page=${nextpage}`">다음 &rarr;</a>
            </li>
          </ul>
        </div>
            ...
            <div class="card-body">
              <div v-for="category in myCategory" :key="category.id">
                <div class="btn-group btn-block">
                  <button type="button" class="btn btn-outline-secondary text-left d-flex justify-content-between" aria-haspopup="true" aria-expanded="false">
...
export default {
  ...
  methods: {
    ...
    deletewantuser () {
      this.$store.state.tempname = this.$store.state.username
          ...
          axios.delete(`/study/${id}`)
            .then(res => {
              ...
              if (this.$route.query.page * 1 > 1) {
                axios.get(`/${this.$store.state.username}/study?page=${this.$route.query.page}`)
                  .then(res => {
                    this.myStudy = res.data.studyList
                    this.pageData = res.data.pageData
                  })
              } else {
                axios.get(`/${this.$store.state.username}/study`)
                  .then(res => {
                    this.myStudy = res.data.studyList
                    this.pageData = res.data.pageData
                  })
  ...
  created () {
    if (this.$store.state.wantuser) {
      if (this.$route.query.page * 1 > 1) {
        axios.get(`/${this.$store.state.wantuser}/study?page=${this.$route.query.page}`)
          .then(res => {
            this.myStudy = res.data.studyList
            this.pageData = res.data.pageData
          })
      } else {
        axios.get(`/${this.$store.state.wantuser}/study`)
          .then(res => {
            this.myStudy = res.data.studyList
            this.pageData = res.data.pageData
          })
